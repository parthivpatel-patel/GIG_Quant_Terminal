...
def touch_heartbeat(*, ok: bool, detail: str = "", run_id: str | None = None) -> None:
    ...
    _HEARTBEAT["detail"] = _short_detail(detail)
...
def _short_detail(detail: str, limit: int = 120) -> str:
    text = " ".join(str(detail or "").split())
    # Strip absolute Windows paths from operator-facing alerts.
    if "Cannot open file" in text or "IOException" in text:
        return "database busy — keep a single serve process (do not run ingest in parallel)"
    if len(text) > limit:
        return text[: limit - 1] + "…"
    return text
...
def _bar_asof() -> date | None:
    try:
        from gig.data.lake import db_lock, last_bar_date, open_store

        with db_lock(timeout=0.5):
            return last_bar_date(open_store())
    except Exception:
        return None
...
def _freshness(asof: date | None) -> dict[str, Any]:
    ...
    return {
        ...
        "age_days": int(age),
        "level": level,
        "label": "fresh" if age == 0 else f"{age}d old",
    }
...
def _last_trade_run() -> dict[str, Any] | None:
    try:
        ...
        return {"error": _short_detail(f"{type(exc).__name__}: {exc}")}
        ...
        alerts.append(
            {
                ...
                "code": "stale_bars",
                "detail": (
                    f"market data as of {fresh.get('asof') or '—'} "
                    f"({fresh.get('age_days')}d) — run daily_ingest"
                    if fresh.get("asof")
                    else "no market data — run daily_ingest"
                ),
            }
        )
        ...
        alerts.append(
            {
                "level": "warn",
                "code": "aging_bars",
                "detail": f"market data {fresh.get('age_days')}d old ({fresh.get('asof')})",
            }
        )
        ...
        alerts.append(
            {
                "level": "warn",
                "code": "audit_read",
                "detail": str(last["error"]),
            }
        )
    elif last and last.get("blocked"):
        alerts.append(
            {
                "level": "warn",
                "code": "last_run_blocked",
                "detail": "last rebalance blocked by risk gate",
            }
        )
        ...
        alerts.append(
            {
                "level": "warn",
                "code": "no_heartbeat",
                "detail": "paper loop idle — start with scripts/run_24_7.py",
            }
        )
